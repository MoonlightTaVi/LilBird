...
public class BirdListingApi {
    ...
    public ResponseEntity<BirdCardDto> getCard(
            @Schema(
        ...
        ) 
    {
        ...
        card.accept(birdEntry);

        // Collect all alternative names
        service.getNamesOf(birdEntry)
            .stream()
            .forEach(name -> card.accept(name));
    ...
    @GetMapping("/{id}/names")
    public ResponseEntity<List<NameGroup>> synonymsOf(
            @Valid
            @Min(1)
            @PathVariable("id") 
                final long id
        ) 
    {
        ...
        final List<NameGroup> entryList = service.getNamesOf(id);
        return ResponseEntity.ok(entryList);
    }
